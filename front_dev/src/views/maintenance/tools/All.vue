...
<script>
    ...
    export default {
      ...
      data: () => {
          return {
              table_settings:{
                  api_url:'maintenance/tool_types/all',
                  delete_url:'maintenance/tool_types/delete',
                  fields:[
                      {key: 'description', label:'Tool Name'},
      ...
      created(){
        bus.$on('serverSidePaginatedTableDbClick', (data)=>{
            this.$router.push('/maintenance/tools/update/'+data.id);
        });
      }
    }
</script>
