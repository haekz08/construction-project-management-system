...
<script>
    ...
    export default {
      ...
      data: () => {
          return {
              table_settings:{
                  api_url:'maintenance/equipment_types/all',
                  delete_url:'maintenance/equipment_types/delete',
                  fields:[
                      {key: 'description', label:'Supplier Name'},
      ...
      created(){
        bus.$on('serverSidePaginatedTableDbClick', (data)=>{
            this.$router.push('/maintenance/equipments/update/'+data.id);
        });
      }
    }
</script>
