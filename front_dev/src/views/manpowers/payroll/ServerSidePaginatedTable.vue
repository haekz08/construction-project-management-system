                    ...
                    <b-col class="text-right">
                        <router-link to="add" v-if="top_right_btn_redirect">
                            <b-button variant="secondary" size="sm">
                                <font-awesome-icon icon="plus" /> Process Payroll
                            </b-button>
                        </router-link>
                ...
                >
                    ...
                    <template v-slot:cell(action)="data">
                        <div class="text-center">
                            <router-link :to="{ name: 'PayrollPrint', params: { id: data.item.id }}">
                            <b-button
                                    variant="primary"
                                    size="sm"
                            >
                                <font-awesome-icon icon="print" /> Print per Manpower
                            </b-button>
                            </router-link>
                            <router-link :to="{ name: 'PayrollPrintPerItem', params: { id: data.item.id }}">
                                <b-button
                                        variant="primary"
                                        size="sm"
                                        class="ml-2"
                                >
                                    <font-awesome-icon icon="print" /> Print per Item
                                </b-button>
                            </router-link>
                        </div>
                    </template>
                    ...
                    <template v-slot:cell(date_from_to)="data">
                        <div class="h5 mb-0">{{data.item.project.title}}</div>
                        <div class="small text-muted">
                            {{data.item.date_from.other_formats.format_1}} - {{data.item.date_to.other_formats.format_1}}
                        </div>
                    </template>
                    <template v-slot:cell(remarks)="data">
                        <div class="h5 mb-0">{{data.item.remarks}}</div>
                        <div class="small text-muted">
                            Authorized By : {{data.item.authorized.full_name}}
                        </div>
                    </template>
                </b-table>
