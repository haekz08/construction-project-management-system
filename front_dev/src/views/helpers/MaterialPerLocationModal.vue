  ...
  <div>
    <div class="card bg-light mb-0">
      <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
          <h6 class="mb-0"><font-awesome-icon icon="search" /> Material Search</h6>
      </div>
      <div class="card-body bg-white position-relative p-2">
        ...
        <div class="list-group bg-white mt-2" style="max-height:260px;overflow-y: auto;" v-if="results.length>0">
            <a href="#" class="list-group-item list-group-item-action" v-for="result in results" @click.prevent="select(result)">
                {{result.name}}<br>
                <small>Balance : <number-format-component :currency="false" :input_value="Number(result.balance)"/> {{result.description}}
                    | Price : <number-format-component :input_value="Number(result.price)"/>/{{result.description}}</small>
            </a>
        </div>
      </div>
    </div>
...
<script>
    ...
    export default {
      ...
      components: {NumberFormatComponent},
        props:{
            project_location_structure_item_id:Number
        },
      data() {
        return {
          is_saving:false,
      ...
      methods: {
        search(){
            this.axios.get('maintenance/materials/search-per-location?project_location_structure_item_id='+this.project_location_structure_item_id+'&search=' + this.search_key).then(response => {
