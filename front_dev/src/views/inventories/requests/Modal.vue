  ...
  <div>
    <div class="card bg-light mb-0">
      <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
          <h6 class="mb-0"><font-awesome-icon icon="plus-circle" /> Material Request</h6>
      </div>
          ...
          <div class="card bg-light mb-0 mt-3">
            ...
            <div class="card-body bg-white position-relative p-2">
              <table class="table border mb-0">
                ...
                <tbody>
                  <tr v-for="(material_request_detail, index) in form.material_request_details">
                    <th scope="row">{{material_request_detail.material.name}}</th>
                    <td>
                      <b-form-input
                    ...
                    <td class="text-center" v-if="editable">
                      <b-button type="submit" size="sm" variant="danger" @click="deleteMaterial(index)"><font-awesome-icon icon="minus-circle" /> Remove</b-button>
                    </td>
                  </tr>
                </tbody>
              </table>
    ...
    export default {
      ...
      watch: {
        material_request:{
            immediate: true,
            handler(){
              if(typeof this.material_request.id != 'undefined')
                this.form = this.material_request;
            }
        },
        form:{
            immediate: true,
      ...
      methods: {
        save(){
          ...
          if(this.form.material_request_details.length<=0){
          ...
          this.swalConfirmSubmit((data)=>{
            ...
            if (data.value) {
              this.is_saving=true;
              this.axios.post('inventory/requests/save',this.form).then(response => {
        ...
        selectMaterial(material){
          if(this.form.material_request_details.find(x => (x.material_id === material.id)))
          {
          ...
          this.form.material_request_details.push(data);
          this.material_modal=false;
        },
        deleteMaterial(index){
          this.swalConfirmDelete((data)=>{
              if (data.value) {
                this.form.material_request_details.splice(index, 1);
              }
          });
        }
      }
