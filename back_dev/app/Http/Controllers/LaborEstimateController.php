...
use App\LaborEstimate;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Validator;

class LaborEstimateController extends Controller
{
    protected $request;
    protected $search='';
    protected $per_page;
    ...
    public function save(){
        ...
        $validate = LaborEstimate::where('project_location_structure_item_id',$data['project_location_structure_item_id'])
            ->where('job_description_id',$data['job_description_id'])
        ...
        $result = LaborEstimate::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response($result,200);
    }
    public function delete(){
        $data = $this->request->all();
        $result = LaborEstimate::find( $data['id'] );
