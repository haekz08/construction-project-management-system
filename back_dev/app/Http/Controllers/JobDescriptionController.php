...
class JobDescriptionController extends Controller
{
    ...
    public function save(){
        ...
        if (isset($data['id']) && $data['id'] != -1) {
            $validate = JobDescription::where('description',$data['description'])
                                      ->where('id','!=',$data['id'])
                                      ->first();
            if($validate)
                return response('Job description already exists.',422);
            $data['updated_by'] = $this->getUserId();
        } else {
            $validate = Validator::make($data, [
                'description' => 'required|unique:job_descriptions',
            ]);
            if ($validate->fails())
                return response('Job description already exists.',422);
        ...
        $result = JobDescription::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response($result,200);
    }
    public function delete(){
        $data = $this->request->all();
        $result = JobDescription::find( $data['id'] );
