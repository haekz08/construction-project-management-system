...
class MaterialUnitController extends Controller
{
    ...
    public function all(){
        ...
        {
            $data = MaterialUnit::where('description', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
    ...
    public function save(){
        ...
        if (isset($data['id']) && $data['id'] != -1) {
            $validate = MaterialUnit::where('description',$data['description'])
                                      ->where('id','!=',$data['id'])
                                      ->first();
            if($validate)
                return response('Material unit description already exists.',422);
            $data['updated_by'] = $this->getUserId();
        } else {
            $validate = Validator::make($data, [
                'description' => 'required|unique:material_units',
            ]);
            if ($validate->fails())
                return response('Material unit description already exists.',422);
        ...
        $result = MaterialUnit::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response($result,200);
    }
    public function delete(){
        $data = $this->request->all();
        $result = MaterialUnit::find( $data['id'] );
