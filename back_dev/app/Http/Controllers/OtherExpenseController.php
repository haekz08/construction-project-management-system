...
class OtherExpenseController extends Controller
{
    ...
    public function all(){
        ...
        {
            $data = OtherExpense::where('description', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
            $data = OtherExpense::all();
        }
    ...
    public function save(){
        ...
        if (isset($data['id']) && $data['id'] != -1) {
            $validate = OtherExpense::where('description',$data['description'])
                                      ->where('id','!=',$data['id'])
                                      ->first();
            if($validate)
                return response('Expense description already exists.',422);
            $data['updated_by'] = $this->getUserId();
        } else {
            $validate = Validator::make($data, [
                'description' => 'required|unique:other_expenses',
            ]);
            if ($validate->fails())
                return response('Expense description already exists.',422);
        ...
        $result = OtherExpense::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response($result,200);
    }
    public function delete(){
        $data = $this->request->all();
        $result = OtherExpense::find( $data['id'] );
