...
class MaterialEstimateController extends Controller
{
    ...
    public function save(){
        ...
        $validate = MaterialEstimate::where('project_location_structure_item_id',$data['project_location_structure_item_id'])
        ...
        $result = MaterialEstimate::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response($result,200);
    }
    public function delete(){
        $data = $this->request->all();
        $result = MaterialEstimate::find( $data['id'] );
